...
export default class Inbox {
  ...
  public async waitForEmailsFrom(
    ...
    while (Date.now() - startTime < timeout) {
      ...
      const newEmails = emails.filter(
        (em) => already.findIndex((a) => a.id === em.id) < 0
      );
  ...
  public async getEmail(id: string): Promise<Email> {
    ...
    try {
      ...
      throw new Error(
        "Could not retrieve email. Inner error: " +
  ...
  private createSocket() {
    this.websocket = new WebSocket(`${Config.WS_EP}`, {
      headers: {
        "User-Agent": Config.USER_AGENT,
    ...
    this.websocket.on("open", () => {
      this.websocket.send(
        JSON.stringify({ cmd: "sub", channel: this.username })
      );
    });
  }
  ...
  private messageHandler(data: string) {
    ...
    if (jsonData.msgs) {
      for (let msg of jsonData.msgs) {
        this.addEmailHeader(msg);
      }
	  this.resolvePromise();
    } else if (jsonData.ping) {
  ...
  private addEmailHeader(mailinatorHeader: MailinatorEmailHeader | any) {
